use std::{collections::HashMap, num::TryFromIntError, path::Path, fmt::Display};
...
#[derive(Debug)]
pub enum Error {
    ParseFromIntError(std::num::ParseIntError),
...
impl Display for Error {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Error::ParseFromIntError(e) => write!(f, "{}", e),
            Error::InvalidCharEncoding(c) => write!(f, "Attempted to convert a invalid value of {} to a char", c),
        }
    }
}

/// [Tokenizer] is the main structure of the library. It is in charge of tokenizing strings of text and decoding them back.
/// It is created from a json file generated by NovelAI which are obtainable from [here](https://github.com/NovelAI/nai-js-tokenizer/tree/main/tokenizer_files).
/// 
/// ## Usage
/// ```
/// use nai_tokenizer::Tokenizer;
/// let tokenizer = Tokenizer::from_path("data/nerdstash_tokenizer_v2.json").expect("Failed to read tokenizer file");
/// let tokens: Vec<u32> = tokenizer.encode("Hello, World!").expect("Failed to encode text");
/// assert_eq!(tokens, [13071, 49231, 2553, 49338]);
...
impl Tokenizer {
    ...
    /// Encode a string using the BPE algorithm.
    pub fn encode(&self, data: &str) -> Result<Vec<u32>, Error> {
    ...
    /// Decode a slice of tokens into a string.
    pub fn decode(&self, tokens: &[u32]) -> Result<String, Error> {
        ...
        for token in tokens {
            ...
            if let Some(stripped) = str.strip_prefix("0x") {
                let result = u32::from_str_radix(stripped, 16);
                if let Ok(c) = result {
                    accumulated_bytes.push(c);
                } else if let Err(e) = result {
                    return Err(Error::ParseFromIntError(e))
                }
            } else {
                if !accumulated_bytes.is_empty() {
                    for byte in &accumulated_bytes {
                        if let Some(c) = char::from_u32(*byte) {
                            ...
                            return Err(Error::InvalidCharEncoding(*byte))
                        }
                    }
        ...
        Ok(if self.bytes_encoder.is_none() {
            text.chars()
                .flat_map(|x| {
                    self.char_to_byte
                        .get(&x.to_string())
    ...
    /// Get all tokens that contain a given string.
    ...
    fn make_unitrim(&self) -> Result<Vec<i32>, Error> {
        ...
        for i in 0..self.encoder.len() {
            ...
            if self.bytes_encoder.is_some() {
                if let Some(stripped) = v.strip_prefix("0x") {
                    if let Ok(c) = u8::from_str_radix(stripped, 16) {
                        bytes.push(c);
                    } else if let Err(e) = u8::from_str_radix(stripped, 16) {
                        return Err(Error::ParseFromIntError(e));
                    }
                } else {
...
mod tests {
    ...
    fn test_nerdstash2() {
        ...
        assert_eq!(result.unwrap_or_else(|e| panic!("Error: {}", e)), "Hello, world!\nI have a huge love for you all.");
    }
    #[test]
    fn test_nerdstash1() {
